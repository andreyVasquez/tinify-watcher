...
**A) Mediante variables de entorno (Recomendado):**
```bash
export TINYPNG_API_KEY="tu_api_key_aqui"
```

**B) Editando directamente el archivo `index.js`:**
Abre `index.js` y modifica la constante `CONFIG`:
```javascript
const CONFIG = {
  ...
  DELETE_ORIGINAL: false // Cambia a true si quieres borrar el archivo original tras comprimirlo
};
```
